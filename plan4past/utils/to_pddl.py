...
from FDgrounder.pddl import TypedObject, Conjunction, Disjunction, UniversalCondition, ExistentialCondition, Atom, NegatedAtom, \
    Action, PropositionalAction, Truth, Effect, Predicate
...
def _(action: Action) -> str():
    ...
    return action_str.format(name=action.name,
...
def _(predicate: Predicate) -> str():
    predicate_string = f'({predicate.name} {" ".join([to_pddl(p) for p in predicate.arguments])})'
