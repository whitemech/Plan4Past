...
MARKER_CONFIGS = dict(markersize=5.0, markeredgewidth=0.5, markeredgecolor=(0.0, 0.0, 0.0, 1))
...
@click.option(
    "--xlabel", type=str, required=True
)
@click.option(
    "--ylabel", type=str, required=True
)
def main(benchmark_dir: str, output: str, title: str, timeout: int, xlabel: str, ylabel: str):
    ...
    dataframes = [pd.read_csv(str(tsv_file), sep="\t") for tsv_file in tool_to_tsv.values()]
    ...
    for idx, df in enumerate(dataframes):
        ...
        nb_node_expanded = np.append(df["nb_node_expanded"].values, [None] * (max_nb_rows - nb_rows))
    ...
    for idx, label in enumerate(labels):
        ...
        plt.plot(x_axis, cactus[:, idx], label=tool_name, color=tool_color, linestyle="-", linewidth=line_width(label), marker=tool_marker, **MARKER_CONFIGS)
