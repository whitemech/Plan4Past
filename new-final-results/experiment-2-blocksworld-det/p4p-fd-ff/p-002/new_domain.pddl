(define (domain blocks-domain)
    ...
    (:types block)
    (:predicates (clear ?x) (emptyhand) (holding ?x) (on ?x ?y) (ontable ?x) (val_on_a_b) (val_on_c_a) (val_on_d_c) (val_on_d_c-and-on_c_a-and-on_a_b))
    (:derived (val_on_a_b) (on a b))
     (:derived (val_on_c_a) (on c a))
     (:derived (val_on_d_c) (on d c))
     (:derived (val_on_d_c-and-on_c_a-and-on_a_b) (and (val_on_d_c) (val_on_c_a) (val_on_a_b)))
    (:action pick-up
        :parameters (?x)
