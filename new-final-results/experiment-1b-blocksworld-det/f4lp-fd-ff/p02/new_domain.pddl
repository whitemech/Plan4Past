(define (domain blocks-domain)
	(:requirements :strips)
	(:types block)
	(:predicates (ontable ?x - block) (on ?x - block ?y - block) (clear ?x - block) (emptyhand) (holding ?x - block) (q1 ?x-00 - block ?y-01 - block) (q2 ?x-00 - block ?y-01 - block) (turnDomain))
	(:action pick-up
		:parameters (?x - block)
	...
	(:action trans-0
		:parameters (?x-00 - block ?y-01 - block)
		:precondition (and (q1 ?x-00 ?y-01) (not (on ?x-00 ?y-01)) (not (turnDomain)))
		:effect (and (q1 ?x-00 ?y-01) (not (q2 ?x-00 ?y-01)) (turnDomain))
	)
	(:action trans-1
		:parameters (?x-00 - block ?y-01 - block)
		:precondition (and (or (and (q1 ?x-00 ?y-01) (on ?x-00 ?y-01)) (q2 ?x-00 ?y-01)) (not (turnDomain)))
		:effect (and (q2 ?x-00 ?y-01) (not (q1 ?x-00 ?y-01)) (turnDomain))
	)
)