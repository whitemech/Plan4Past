(define (domain blocks-domain)
	(:requirements :non-deterministic :equality :typing)
	(:constants )
	(:types block qstate)
	(:predicates (holding ?b - block) (emptyhand) (on-table ?b - block) (on ?b1 ?b2 - block) (clear ?b - block) (prev_autstate ?q - qstate) (is_accepting ?q - qstate) (dummy_goal))
	(:action pick-up
		:parameters (?b1 ?b2 - block)
		:precondition (and (emptyhand) (clear ?b1) (on ?b1 ?b2) (not (= ?b1 ?b2)))
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(oneof
			 (and (holding ?b1) (clear ?b2) (not (emptyhand)) (not (clear ?b1)) (not (on ?b1 ?b2)))
			 (and (clear ?b2) (on-table ?b1) (not (on ?b1 ?b2)))
			))
	)
	(:action pick-up-from-table
		:parameters (?b - block)
		:precondition (and (emptyhand) (clear ?b) (on-table ?b) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(oneof
			 (and )
			 (and (holding ?b) (not (emptyhand)) (not (on-table ?b)))
			))
	)
	(:action put-on-block
		:parameters (?b1 ?b2 - block)
		:precondition (and (holding ?b1) (clear ?b2) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(oneof
			 (and (on ?b1 ?b2) (emptyhand) (clear ?b1) (not (holding ?b1)) (not (clear ?b2)))
			 (and (on-table ?b1) (emptyhand) (clear ?b1) (not (holding ?b1)))
			))
	)
	(:action put-down
		:parameters (?b - block)
		:precondition (and (holding ?b) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) (on-table ?b) (emptyhand) (clear ?b) (not (holding ?b)))
	)
	(:action pick-tower
		:parameters (?b1 ?b2 ?b3 - block)
		:precondition (and (emptyhand) (on ?b1 ?b2) (on ?b2 ?b3) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(oneof
			 (and )
			 (and (holding ?b2) (clear ?b3) (not (emptyhand)) (not (on ?b2 ?b3)))
			))
	)
	(:action put-tower-on-block
		:parameters (?b1 ?b2 ?b3 - block)
		:precondition (and (holding ?b2) (on ?b1 ?b2) (clear ?b3) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(oneof
			 (and (on ?b2 ?b3) (emptyhand) (not (holding ?b2)) (not (clear ?b3)))
			 (and (on-table ?b2) (emptyhand) (not (holding ?b2)))
			))
	)
	(:action put-tower-down
		:parameters (?b1 ?b2 - block)
		:precondition (and (holding ?b2) (on ?b1 ?b2) )
		:effect (and 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) 
			(when (prev_autstate q0)
			 (and (not (prev_autstate q0)) (prev_autstate q1))
			) 
			(when (and (prev_autstate q1) (not (on b6 b7)))
			 (and )
			) 
			(when (and (prev_autstate q1) (on b6 b7))
			 (and (not (prev_autstate q1)) (prev_autstate q3))
			) 
			(when (prev_autstate q2)
			 (and )
			) 
			(when (and (prev_autstate q3) (not (on b5 b6)))
			 (and )
			) 
			(when (and (prev_autstate q3) (on b5 b6))
			 (and (not (prev_autstate q3)) (prev_autstate q4))
			) 
			(when (and (prev_autstate q4) (not (on b4 b5)))
			 (and )
			) 
			(when (and (prev_autstate q4) (on b4 b5))
			 (and (not (prev_autstate q4)) (prev_autstate q5))
			) 
			(when (and (prev_autstate q5) (not (on b3 b4)))
			 (and )
			) 
			(when (and (prev_autstate q5) (on b3 b4))
			 (and (not (prev_autstate q5)) (prev_autstate q6))
			) 
			(when (and (prev_autstate q6) (not (on b2 b3)))
			 (and )
			) 
			(when (and (prev_autstate q6) (on b2 b3))
			 (and (not (prev_autstate q6)) (prev_autstate q7))
			) 
			(when (and (prev_autstate q7) (not (on b1 b2)))
			 (and )
			) 
			(when (and (prev_autstate q7) (on b1 b2))
			 (and (not (prev_autstate q7)) (prev_autstate q8))
			) 
			(when (prev_autstate q8)
			 (and )
			) 
			(when (prev_autstate q9)
			 (and (not (prev_autstate q9)) (prev_autstate q2))
			) (on-table ?b2) (emptyhand) (not (holding ?b2)))
	)
	(:action end
		:parameters (?q - qstate)
		:precondition (and (prev_autstate ?q) (is_accepting ?q) )
		:effect (dummy_goal)
	)
)
